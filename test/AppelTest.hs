...
testParseAppel14 =
    testCase "parses test14 (read from file)" $ (readFile "test/appeltestcases/test14.tig" >>= (assertEqual [] (Program $ (LetExp {decls = [TypeDecl {tname = "arrtype", ttyp = ArrayType "int"},TypeDecl {tname = "rectype", ttyp = RecordType [Field {fieldname = "name", escape = True, typ = "string"},Field {fieldname = "id", escape = True, typ = "int"}]},VarDecl {vname = "rec", vescape = True, vtyp = Nothing, vinit = RecordExp {fields = [("id",IntExp 0),("name",StrExp "aname")], rtyp = "rectype"}},VarDecl {vname = "arr", vescape = True, vtyp = Nothing, vinit = ArrayExp {atyp = "arrtype", size = IntExp 3, ainit = IntExp 0}}], lbody = IfExp {iftest = OpExp {left = VarExp (SimpleVar "rec"), oper = NotEqual, right = VarExp (SimpleVar "arr")}, thenexp = IntExp 3, elseexp = Just (IntExp 4)}}))) . happyTokenParse . scanner)
testParseAppel15 =
    testCase "parses test15 (read from file)" $ (readFile "test/appeltestcases/test15.tig" >>= (assertEqual [] (Program $ IfExp {iftest = IntExp 20, thenexp = IntExp 3, elseexp = Nothing})) . happyTokenParse . scanner)
testParseAppel16 =
    testCase "parses test16 (read from file)" $ (readFile "test/appeltestcases/test16.tig" >>= (assertEqual [] (Program $ (LetExp {decls = [TypeDecl {tname = "a", ttyp = NameType "c"},TypeDecl {tname = "b", ttyp = NameType "a"},TypeDecl {tname = "c", ttyp = NameType "d"},TypeDecl {tname = "d", ttyp = NameType "a"}], lbody = StrExp ""}))) . happyTokenParse . scanner)
testParseAppel17 =
    testCase "parses test17 (read from file)" $ (readFile "test/appeltestcases/test17.tig" >>= (assertEqual [] (Program $ (LetExp {decls = [TypeDecl {tname = "tree", ttyp = RecordType [Field {fieldname = "key", escape = True, typ = "int"},Field {fieldname = "children", escape = True, typ = "treelist"}]},VarDecl {vname = "d", vescape = True, vtyp = Just "int", vinit = IntExp 0},TypeDecl {tname = "treelist", ttyp = RecordType [Field {fieldname = "hd", escape = True, typ = "tree"},Field {fieldname = "tl", escape = True, typ = "treelist"}]}], lbody = VarExp (SimpleVar "d")}))) . happyTokenParse . scanner)
