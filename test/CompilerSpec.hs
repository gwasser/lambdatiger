...
-- from Appel page 97
testParserSeq =
  testCase "parses '(a := 5; a+1)'" $ assertEqual [] (Program $ SeqExp [AssignExp (SimpleVar "a") (IntExp 5), OpExp (VarExp $ SimpleVar "a") (Add) (IntExp 1)]) (happyTokenParse $ alexScanTokens "(a := 5; a+1)")
testParserArray =
...
testParserLetRecursiveFuncs =
  testCase "parses 'let...' recursive funcs" $ assertEqual [] (Program $ LetExp [VarDecl "a" True Nothing (IntExp 5), FunDecls [FunDecl "f" [] (Just "int") (CallExp "g" [VarExp $ SimpleVar "a"]), FunDecl "g" [Field "i" True "int"] Nothing (CallExp "f" [])]] (CallExp "f" [])) (happyTokenParse $ alexScanTokens "let var a := 5 function f() : int = g(a) function g(i : int) = f() in f() end")
-- from Appel page 99
testParserTypeDecls =
    testCase "parses two type decls" $ assertEqual [] (Program $ LetExp [TypeDecl "tree" (RecordType [Field "key" True "int", Field "children" True "treelist"]), TypeDecl "treelist" (RecordType [Field "head" True "tree", Field "tail" True "treelist"])] (IntExp 0)) (happyTokenParse $ alexScanTokens "let type tree = {key: int, children: treelist} type treelist = {head: tree, tail: treelist} in 0 end")
testParserLetFuncDecl =
